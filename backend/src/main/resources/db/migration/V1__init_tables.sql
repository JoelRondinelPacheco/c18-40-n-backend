...
create table review (event_id bigint not null, user_data_id bigint not null, id bigserial not null, qualification numeric(2,1), comment varchar(500), primary key (id));
...
alter table if exists review add constraint fk_review_user foreign key (user_data_id) references user_data;
